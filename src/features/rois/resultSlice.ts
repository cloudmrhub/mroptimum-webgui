...
export const resultGetters = {
  ...
  getPseudoReplicaCount: (state: RootState): number | undefined => {
    try {
      if (state.result.activeJob?.setup.task?.options["NR"] !== undefined)
  ...
  getBoxSize: (state: RootState): number | undefined => {
    try {
      if (state.result.activeJob?.setup.task?.options["boxSize"] !== undefined)
  ...
  getSignal: (state: RootState): FileReference | undefined => {
    return state.result.activeJob?.setup.task?.options.reconstructor?.options
      .signal;
  },
  ...
  getReconstructionMethod: (state: RootState): number | undefined => {
    try {
      if (
        state.result.activeJob?.setup.task?.options.reconstructor?.id !==
        undefined
      )
        return Number(
          state.result.activeJob?.setup.task?.options.reconstructor?.id,
  ...
  getSensitivityMapSource: (state: RootState): FileReference | undefined => {
    return state.result.activeJob?.setup.task?.options.reconstructor?.options
      .sensitivityMap?.options.sensitivityMapSource;
  },
  ...
  getDecimateAcceleration1: (state: RootState): number | undefined => {
    let acc =
      state.result.activeJob?.setup.task?.options.reconstructor?.options
        .accelerations;
    return acc ? acc[0] : 0;
  },
  ...
  getDecimateACL: (state: RootState): number | null | undefined => {
    let acl =
      state.result.activeJob?.setup.task?.options.reconstructor?.options.acl;
    return acl ? acl[0] : 0;
  },
