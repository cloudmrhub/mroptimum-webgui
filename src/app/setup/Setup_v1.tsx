...
import { getUploadedData } from "cloudmr-ux/core";
import { uploadData } from "cloudmr-ux/core";
...
import { is_safe_twix } from "cloudmr-ux/core";
...
import { UploadedFile } from "cloudmr-ux/core";
import { formatBytes, jobActions } from "cloudmr-ux/core";
...
import { downloadStringAsFile } from "cloudmr-ux/core";
import { SNREditor } from "./SetupEditor";
import { LambdaFile } from "cloudmr-ux";
import { createTheme } from "@mui/material/styles";
import { uploadHandlerFactory } from "cloudmr-ux/core";
...
const Setup = () => {
  ...
  const [openPanel, setOpenPanel] = useState(
    noise !== undefined && signal !== undefined ? [2] : [1],
  );
  ...
  const preflightValidation = () => {
    if (signal === undefined) {
      ...
      setSDWarning(
        "No signal file defined, please make sure signal file has been successfully uploaded.",
      );
      setSDOpen(true);
      return false;
    }
    if (noise === undefined && signal.options.multiraid === false) {
      setSDWarningHeader("Setup validation failed");
      setSDWarning(
        "No noise file defined and signal file is not multi-raid," +
    ...
    if (
      ...
      setSDWarningHeader("Setup validation failed");
      setSDWarning(
        "No mask file defined yet upload mask option was selected," +
    ...
    <Fragment>
      ...
      >
        <CmrPanel header="Job Queue" className={"mb-2"} key={"0"}>
          ...
          <SNREditor
            ...
            edit={() => {
              ...
              if (editActive && analysisMethod !== undefined) {
                setSNREditWarning(
                  "Consider queuing the currently " +
            ...
            confirm={() => {
              if (editing === rowId)
                // when confirming both the snr edit and the name change
                dispatch(
                  setupSetters.completeSNREditing({
                    id: editing,
                    alias: editAlias,
                  }),
                );
              else {
                // when confirming the name change alone
          ...
          <CmrButton
            ...
            onClick={() => {
              ...
              if (jobSelectionModel.length === 0) {
                setSDWarning(
                  "Please select the jobs that you would like to submit.",
                );
          ...
          <CmrButton
            sx={{ width: "49%", marginLeft: "1%", mt: 1 }}
            variant={"contained"}
            color={"error"}
            onClick={() => {
              if (jobSelectionModel.length === 0) {
                setSDWarning(
                  "Please select the jobs that you would like to delete.",
                );
                setSDWarningHeader("No Job Selected for Deletion");
                setSDOpen(true);
              } else {
                setSNRDeleteWarning(
                  `You are about to delete Job ${jobSelectionModel}.`,
                );
                setSNRDeleteOpen(true);
                setSNRDeleteWarningCallback(() => {
                  // @ts-ignore
                  return () =>
                    dispatch(
                      setupSetters.bulkDeleteQueuedJobs(jobSelectionModel),
                    );
                });
              }
            }}
        ...
        <CmrPanel
          key="2"
          header={editing === -1 ? "SNR Set Up" : `Editing Job ${editing}`}
          className="mb-2"
        >
          <FormControl
            style={{ width: "100%" }}
              ...
              >
                ...
                {reconstructionMethod !== undefined && (
                  <CmrPanel
                    ...
                    expanded={true}
                    className={" border-0"}
                    cardProps={{ className: "ms-0 me-0 mt-0 mb-0" }}
                  >
                    <CmrCheckbox
                      className="me-2 ms-1"
                      ...
                      onChange={(event) => {
                        dispatch(
                          setupSetters.setFlipAngleCorrection(
                        ...
                        <Fragment>
                          <FormControl
                            onChange={(event) => {
                              //@ts-ignore
                              dispatch(
                                setupSetters.setLoadSensitivity(
                            ...
                            >
                              ...
                              {loadSensitivity ? (
                                <CMRSelectUpload
                                  ...
                                  onSelected={(file) => {
                                    dispatch(
                                      setupSetters.setSensitivityMapSource(
                                        file,
                                      ),
                                    );
                                  }}
                                ...
                                >
                                  <InputLabel id="css-label">
                                    Coil sensitivities calculation method
                                  </InputLabel>
                                  <Select
                                    labelId="css-label"
                                  ...
                                  >
                                    {["inner", "innerACL"].map(
                                      (value, index) => {
                                        return secondaryToCoilMethodMaps[
                                          reconstructionMethod
                                        ].indexOf(value) >= 0 ? (
                                          <MenuItem value={value}>
                                            {coilOptionAlias[value]}
                                          </MenuItem>
                                        ) : undefined;
                                      },
                                    )}
                    ...
                    {decimateMapping[reconstructionMethod] && (
                      <Fragment>
                        <CmrCheckbox
                          ...
                          onChange={(event) =>
                            dispatch(
                              setupSetters.setDecimate(event.target.checked),
                            )
                          }
                        >
                          Decimate Data
                    ...
                    <Row>
                      ...
                      {[2].indexOf(reconstructionMethod) >= 0 && (
                        <CmrCheckbox
                          ...
                          onChange={(e) => {
                            dispatch(
                              setupSetters.setOutputGFactor(e.target.checked),
                            );
                          }}
                          defaultChecked={true}
